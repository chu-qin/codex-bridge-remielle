...
- 支持拖拽、位置记忆和 20%～300% 缩放
- 支持 Codex Hooks、JSONL 兼容监听，以及 Claude Code 状态与 Token 统计
- 提供悬停级联菜单和 Windows 系统托盘
...
默认只在任务期间显示。右键菜单为**悬停级联**：常驻、查看后隐藏、桌宠大小、状态与 Token、更多设置均在一级菜单，带子菜单的条目只需鼠标悬停即展开（无需逐级点击），空间不足时自动翻转到另一侧。桌宠大小保留常用比例并显示当前大小，完整范围使用滚轮调整；“状态与 Token”的显示开关与位置设置已提升到一级菜单。
